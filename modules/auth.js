// ============================================================
//  modules/auth.js  — v1.0
//  Gestión de wallets autorizadas para acceder al panel admin.
//
//  MODELO DE AUTORIZACIÓN (dos capas):
//
//  Capa 1 — On-chain (autoridad máxima, inmutable desde aquí):
//    La wallet que es owner del contrato SIEMPRE tiene acceso,
//    independientemente de la lista local. Esta verificación
//    usa contract.owner() y no puede ser manipulada localmente.
//
//  Capa 2 — Lista local (delegable, configurable desde el panel):
//    El owner puede agregar otras wallets a una lista guardada
//    en localStorage. Estas wallets también pueden acceder al panel.
//    El owner puede agregar y eliminar wallets de esta lista.
//
//  IMPORTANTE:
//    La lista local es una conveniencia operativa, no una garantía
//    de seguridad criptográfica. Un usuario avanzado podría editar
//    localStorage desde DevTools. Sin embargo, el contrato en sí
//    solo puede ser modificado por el owner on-chain, así que
//    el acceso al panel solo permite leer y escribir en el contrato
//    con la wallet conectada — si no es owner, las txs revertirán.
//    El acceso oculto es una medida de UX, no de seguridad crítica.
//
//  USO:
//    import { isAuthorized, getAuthList, addAuthWallet, removeAuthWallet } from '../modules/auth.js';
// ============================================================

const STORAGE_KEY = 'adminAuthList';

// ============================================================
//  getAuthList()
//  Retorna el array de wallets autorizadas (en minúsculas).
//  Siempre retorna un array, nunca null.
// ============================================================
export function getAuthList() {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed)) return [];
    // Normalizar a minúsculas y filtrar entradas inválidas
    return parsed
      .filter(w => typeof w === 'string' && w.startsWith('0x') && w.length === 42)
      .map(w => w.toLowerCase());
  } catch {
    return [];
  }
}

// ============================================================
//  saveAuthList()
//  Guarda el array de wallets autorizadas.
//  @param {string[]} list — array de wallets en minúsculas
// ============================================================
function saveAuthList(list) {
  const clean = list
    .filter(w => typeof w === 'string' && w.startsWith('0x') && w.length === 42)
    .map(w => w.toLowerCase());
  // Deduplicar
  const unique = [...new Set(clean)];
  localStorage.setItem(STORAGE_KEY, JSON.stringify(unique));
  return unique;
}

// ============================================================
//  addAuthWallet()
//  Agrega una wallet a la lista autorizada.
//  @param {string} wallet — dirección 0x...
//  Retorna { ok: bool, list: string[], reason?: string }
// ============================================================
export function addAuthWallet(wallet) {
  if (!wallet || typeof wallet !== 'string') {
    return { ok: false, reason: 'Dirección inválida.' };
  }
  const w = wallet.toLowerCase().trim();
  if (!w.startsWith('0x') || w.length !== 42) {
    return { ok: false, reason: 'Dirección inválida. Debe ser 0x seguido de 40 caracteres.' };
  }

  const current = getAuthList();
  if (current.includes(w)) {
    return { ok: false, reason: 'Esta wallet ya está en la lista.' };
  }

  const updated = saveAuthList([...current, w]);
  return { ok: true, list: updated };
}

// ============================================================
//  removeAuthWallet()
//  Elimina una wallet de la lista autorizada.
//  @param {string} wallet
//  Retorna { ok: bool, list: string[] }
// ============================================================
export function removeAuthWallet(wallet) {
  const w       = wallet.toLowerCase().trim();
  const current = getAuthList();
  const updated = saveAuthList(current.filter(x => x !== w));
  return { ok: true, list: updated };
}

// ============================================================
//  isInAuthList()
//  Comprueba si una wallet está en la lista local.
//  @param {string} wallet
//  @returns {boolean}
// ============================================================
export function isInAuthList(wallet) {
  if (!wallet) return false;
  return getAuthList().includes(wallet.toLowerCase().trim());
}

// ============================================================
//  isAuthorized()
//  Verificación completa: lista local O owner on-chain.
//  Requiere sesión activa (getSession().contract no null).
//
//  @param {string} wallet   — dirección de la wallet conectada
//  @param {object} contract — instancia ethers del contrato
//  @returns {Promise<{ authorized: bool, isOwner: bool, reason: string }>}
// ============================================================
export async function isAuthorized(wallet, contract) {
  if (!wallet || !contract) {
    return { authorized: false, isOwner: false, reason: 'Sin sesión activa.' };
  }

  const w = wallet.toLowerCase().trim();

  // Verificación 1: ¿es el owner on-chain?
  try {
    const contractOwner = await contract.owner();
    if (contractOwner.toLowerCase() === w) {
      return { authorized: true, isOwner: true, reason: 'Owner del contrato.' };
    }
  } catch {
    // Si la llamada on-chain falla, continuar con verificación local
  }

  // Verificación 2: ¿está en la lista local?
  if (isInAuthList(w)) {
    return { authorized: true, isOwner: false, reason: 'Wallet en lista autorizada.' };
  }

  return {
    authorized: false,
    isOwner:    false,
    reason:     'Wallet no autorizada. Solo el owner del contrato o wallets en la lista de acceso pueden ingresar.'
  };
}

// ============================================================
//  clearAuthList()
//  Limpia toda la lista (útil para reset desde el panel).
// ============================================================
export function clearAuthList() {
  localStorage.removeItem(STORAGE_KEY);
}

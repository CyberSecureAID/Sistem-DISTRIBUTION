// ============================================================
//  ui/admin.js  — v2.1 (corregido)
//
//  CORRECCIONES v2.1:
//  - _refreshBalances() y transferOwnershipAction() usaban
//    await import('../core/provider.js') dinámicamente pese a
//    que provider.js ya está importado estáticamente en este
//    módulo. Reemplazado por llamadas directas a getSession().
//  - Coherente con los cambios de provider.js v2.1:
//    tryReconnect() puede devolver { wrongNetwork } — se maneja
//    mostrando el banner de red sin bloquear el panel.
// ============================================================

import { connectWallet, tryReconnect, verifyOwner, getBalance, watchWalletEvents, getSession } from '../core/provider.js';
...
import { saveConfig, loadConfig, ACTIONS } from '../modules/config.js';
...
export function initAdmin() {
  ...
  tryReconnect().then(session => {
    ...
    // Red incorrecta detectada en reconexión silenciosa
    ...
    if (session.ready) _onSessionReady(session);
  }).catch(() => {});
...
// ============================================================
//  REFRESCAR SALDOS
//  CORRECCIÓN: usa getSession() directamente (import estático)
//  en lugar de await import() dinámico.
// ============================================================
async function _refreshBalances() {
  try {
    const { account, contract } = getSession();
    if (!account) return;
...
// ============================================================
//  TRANSFER OWNERSHIP
//  CORRECCIÓN: usa getSession() directamente.
// ============================================================
export async function transferOwnershipAction() {
  const { contract } = getSession();
  if (!contract) { _log('No hay sesión activa.', 'warn'); return; }
